...
// === EVENTOS DE RESIZE ===
window.addEve// Registro de plugins GSAP
gsap.registerPlugin(ScrollTrigger);
...
window.addEventListener('error', function (event) {
  console.error('Error// Registro de plugins GSAP
gsap.registerPlugin(ScrollTrigger);
...
window.addEventListener('error', function (event) {
  console.error('Error capturado:', event.message, 'en', event.filename, 'línea', event.lineno);
}); capturado:', event.message, 'en', event.filename, 'línea', event.lineno);
});ntListener('resize', debounce(() => {
  ScrollTrigger.refresh();
}, 250));
